...
class MonitorDB:
    ...
    def start_run(self) -> int:
        """Create a new monitor run row. Returns run ID."""
    ...
    def insert_snapshot(self, run_id: int, data: dict) -> int:
        """Insert a market snapshot row. Returns snapshot ID."""
        cur = self.conn.execute("""
    ...
    def upsert_latest(self, data: dict):
        """Upsert the latest market stats for a product."""
        self.conn.execute("""
    ...
    def get_exchange_rate_history(self, limit: int = 10) -> list[dict]:
        """Get recent exchange rates for fallback."""
