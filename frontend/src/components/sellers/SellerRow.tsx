...
import { Star, CheckCircle2, XCircle, Megaphone } from 'lucide-react';
...
export function SellerRow({ seller, rank, isCheapest }: SellerRowProps) {
  return (
    <tr className={`border-b border-slate-100 ${isCheapest ? 'bg-emerald-50/50' : ''}`}>
      <td className="px-4 py-3">
        <div className={`flex items-center justify-center w-7 h-7 rounded-full text-sm font-semibold ${
          isCheapest ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
        }`}>
          {rank}
        </div>
      </td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          {isCheapest && <Badge variant="success">ارزان‌ترین</Badge>}
          {seller.is_advertisement && (
